defmodule AshStorage.Changes.HandleDependentAttachments do
  ...
  defp mark_for_purge(attachments, context_opts) do
    op_opts = Keyword.take(context_opts, [:actor, :tenant, :authorize?, :tracer])

    Enum.reduce_while(attachments, {:ok, []}, fn att, {:ok, acc} ->
      blob = att.blob

      with {:ok, _} <-
             Ash.destroy(att, Keyword.merge(op_opts, action: :destroy, return_destroyed?: true)),
           {:ok, blob} <-
             Ash.update(
               blob,
               %{pending_purge: true},
               Keyword.merge(op_opts, action: :mark_for_purge, return_record?: true)
             ) do
        {:cont, {:ok, [blob | acc]}}
      else
        {:error, error} -> {:halt, {:error, error}}
  ...
  defp destroy_and_collect_keys(attachments, context_opts) do
    ...
    Enum.reduce_while(attachments, {:ok, []}, fn att, {:ok, keys_acc} ->
      ...
      loaded_blob = Ash.load!(blob, :parsed_service_opts, op_opts)
      ctx = AshStorage.Service.Context.new(loaded_blob.parsed_service_opts || [])

      with {:ok, _} <-
             Ash.destroy(att, Keyword.merge(op_opts, action: :destroy, return_destroyed?: true)),
           {:ok, _} <-
             Ash.destroy(blob, Keyword.merge(op_opts, action: :destroy, return_destroyed?: true)) do
  ...
  defp destroy_attachment_records(attachments, context_opts) do
    ...
    Enum.reduce_while(attachments, {:ok, []}, fn att, {:ok, acc} ->
      case Ash.destroy(att, Keyword.merge(op_opts, action: :destroy, return_destroyed?: true)) do
